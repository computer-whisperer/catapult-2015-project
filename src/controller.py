...
class Controller(threading.Thread):
    ...
    def run(self):
        ...
        self.world_frame = ttk.Frame(self.mainframe, width="300", height="300", borderwidth=5, relief="groove")
        self.world_frame.grid(column=0, row=1)

        self.world_stats = ttk.Label(self.world_frame, text="", width=width)
        self.world_stats.grid(column=0, row=0)
    ...
    def display_world_stats(self):
        self.world_stats.configure(text="World Stats: \n" + "\n".join("{}: {}".format(key, self.world.stats[key]) for key in self.world.stats))

    def tick(self):
        if self.world.selected_agent is not None:
            self.display_agent_stats(self.world.selected_agent)
        self.display_world_stats()
        self.root.after(100, self.tick)
